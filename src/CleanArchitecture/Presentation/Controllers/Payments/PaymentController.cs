...
public class PaymentController : IPaymentController
{
    ...
    {
        _createPaymentUseCase = CreatePaymentUseCase.Create(paymentGateway, orderGateway);
    ...
    {
        var payment = await _createPaymentUseCase.ExecuteAsync(paymentRequest, cancellationToken);
    ...
    {
        var payment = await _confirmPaymentUseCase.ExecuteAsync(id, cancellationToken);
        return _paymentPresenter.Convert(payment);
    }
